<!-- $Revision: 1.7.2.1 $ $Date: 2012/06/19 16:24:56 $ -->
<!--
#(c)#=====================================================================
#(c)#
...
<xsl:otherwise>
<xsl:if test="(string-length(page:label) or string-length(page:value)) and (position()='1' or position()='2')">
<div class="line{position()}Link">
<xsl:value-of select="page:label"/>&#160;<xsl:value-of select="page:value"/>
</div>
</xsl:if>
...
<div class="pubYear">
<xsl:for-each select="page:element[contains(@nameId,'page.searchResults.contents.line')]">
<xsl:if test="@nameId='page.searchResults.contents.line3'">
<xsl:if test="string-length(page:label) or string-length(page:value)">
<xsl:value-of select="page:label"/>&#160;<xsl:value-of select="page:value"/>
</xsl:if>
</xsl:if>
...
<div class="callAvailWrap">

<xsl:for-each select="page:element[contains(@nameId,'page.searchResults.contents.line')]">

<xsl:choose>
<xsl:when test="@nameId='page.searchResults.contents.line4'">
<div class="callNo">
<xsl:if test="string-length(page:label) or string-length(page:value)">
<xsl:value-of select="page:label"/>&#160;<xsl:value-of select="page:value"/>
</xsl:if>
...
<xsl:choose>
<xsl:when test="@nameId='page.searchResults.contents.line1'"/>
<xsl:when test="@nameId='page.searchResults.contents.line2'"/>
<xsl:when test="@nameId='page.searchResults.contents.line5'">
<xsl:value-of select="page:label"/>
</xsl:when>
<xsl:when test="@nameId='page.searchResults.contents.line4'">
            ...
            <ul class="navbar">
                ...
                <xsl:for-each select="page:element[@nameId='page.searchResults.editSearch.link']">
                    <li>
                        ...
                        <input id="{page:buttonAction}" class="yellowBtn" type="submit" name="{page:buttonAction}" value="{page:buttonText}" />
                        <span class="yellowBtnRight">&#160;</span>
                        <span>&#160;&#160;Use &#34;Edit Search&#34; to retain search limits during your session</span>
                    </li>
                </xsl:for-each>
